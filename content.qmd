...
# Probabilistic classifier training

Our goal is to train a probabilistic classifier, by which we mean one which returns calibrated probabilistic predictions (see, e.g., [@gneiting2007probabilistic]).  Roughly speaking, calibration means that if the system says there's an 80% chance of an item being a category, then there really is an 80% chance of it being that category.  We use frequentist notions of calibration for this, with $N$ predictions of 80% leading to an expected $\textrm{binomial}(N, 0.8)$ distribution over correct items.  

With typical gold-standard training data, each item is assinged a unique category.  With probabilistic training, we instead train with a probability distribution over categories.  In this section, we provide simulations that show why probabilisic training is preferable, and barring that, why sampling a single label from the probability distribution for the training data is preferable to using the most probable category.  

## Logistic regression classifiers

We will use a simple binary logistic regression with a vector of $L$ predictors as a running example, because it is simple, but there is nothing special about this particular classifier or choice of data shape.  If we have $N$ data items, the predictors form a matrix $x \in \mathbb{R}^{N \times L}$, whereas the outcomes take the form of a boolean vector $z \in \{0, 1\}^N$. 

### Data generating model for logistic regression

The data generating model for the true category $z_n \in \{0, 1\}$ for item $n \in 1{:}N$ is based on an intercept parameter $\alpha \in \mathbb{R}$ and slope parameter $\beta \in \mathbb{R}$,
$$
z_n \sim \textrm{bernoulli}\!\left(\textrm{logit}^{-1}\!\left(\alpha + \beta \cdot x_n^{\top}\right)\right),
$$
where $x_n$ is the $n$-th row of the data matrix $x$.
...
Given training data $x, y$, the posterior distribution over the parameters is 
$$
\begin{array}{rcl}
...
\\[6pt]
& = & \displaystyle \int p(\tilde{y} \mid \tilde{x}, \alpha, \beta) \cdot p(\alpha, \beta \mid x, y) \ \textrm{d} (\alpha, \beta).
\end{array}
$$
