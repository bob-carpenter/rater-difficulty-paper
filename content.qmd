...
\,\right]
\\[8pt]
& = & \int \alpha \cdot p(\alpha \mid x, y) \ \textrm{d}\alpha,
\end{array}
$$
...
$$
where $\Sigma$ is a symmetric, positive-definite, $L \times L$ matrix, which is defined by
$$
\Sigma_{i, j} = \rho^{\left|i - j\right|}
$$
for some $\rho \in (-1, 1)$.  Both approaches to simulating predictors produce indibidual predictors that are marginally standard normal.
